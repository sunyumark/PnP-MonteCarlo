...
class FastMRIBrainData(Dataset):
    ...
    ''' The fastMRI brain images are padded to 256 by 256
    '''
...
if __name__ == '__main__':
    ...
    data = FastMRIBrainData(finetune=True, zero_mean=False)
    for i in range(10):
        image = data[i]
        plt.figure()
        plt.subplot(1,2,1)
        plt.imshow(image.permute(1,2,0).numpy(),vmin=0)
        plt.colorbar()
        plt.subplot(1,2,2)
        plt.imshow(data[i].permute(1,2,0).numpy(),vmin=0)
        plt.colorbar()
        plt.savefig(f'./deleteme/fastmri/{i}.png')
        plt.close()